...
import {
  BellRing, Send, Mail, Clock, CheckCircle, XCircle,
  Filter, Search, CalendarDays, ArrowUpDown, FileText,
  Download, RefreshCw, TrendingUp, TrendingDown, Play,
  Loader2,
} from "lucide-react";
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 transition-all duration-200 hover:shadow-md hover:border-foreground/20">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground/60 font-medium">{label}</div>
...
function Badge({ variant, children }: { variant: "sales" | "purchase" | "admin" | "debtor" | "sent" | "failed" | "overdue" | "upcoming"; children: React.ReactNode }) {
  const styles: Record<string, string> = {
    sales: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-800/40",
    purchase: "bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950/30 dark:text-purple-400 dark:border-purple-800/40",
    admin: "bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700",
    debtor: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800/40",
    ...
    failed: "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/30 dark:text-red-400 dark:border-red-800/40",
    overdue: "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/30 dark:text-red-400 dark:border-red-800/40",
    upcoming: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800/40",
  };
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${styles[variant] || styles.sales}`}>
      {children}
    </span>
  );
}
...
function RunAllButton() {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-all hover:bg-primary/90 disabled:opacity-50 shadow-sm"
    >
      {runAll.isPending ? (
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 rounded-lg border border-border bg-background px-4 py-2 text-sm font-medium transition-all hover:bg-accent hover:text-accent-foreground shadow-sm"
      >
...
function InvoiceReminderModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[80vh] w-full max-w-xl overflow-y-auto rounded-xl border border-border bg-card shadow-vault" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-md border border-border bg-background/40 p-1 grid grid-cols-2 gap-1">
            {(["sales", "purchase"] as const).map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => { setMode(m); setSearchInv(""); }}
          ...
          <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
            {(mode === "sales" ? invoicesQ.isLoading : purchasesQ.isLoading) ? (
              ...
              unpaidInvoices.slice(0, 50).map((inv: any) => {
                const dud = inv.due_date ? Math.round((new Date(inv.due_date).getTime() - Date.now()) / 86400000) : 0;
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground truncate">{counterpartyName}</span>
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-xs text-muted-foreground">Due: {fmtDate(inv.due_date)}</span>
                        <span className={`text-xs font-medium ${isOverdue ? "text-destructive" : "text-amber-600"}`}>
                          {isOverdue ? `${Math.abs(dud)}d overdue` : dud === 0 ? "Due today" : `${dud}d left`}
                        </span>
                        <span className="text-xs font-medium num">${Number(inv.amount || 0).toLocaleString()}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => sendReminder.mutate(inv.id)}
                      disabled={sendReminder.isPending}
                      className={`ml-3 inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                        sending
          ...
          <p className="text-[10px] text-muted-foreground text-center pt-2 border-t border-border">
            Showing up to 50 invoices sorted by urgency (closest due first).
            Instant reminders are also triggered automatically when invoices are created or updated.
          </p>
        </div>
      </div>
    </div>
...
function RemindersPage() {
  ...
  const [sortField, setSortField] = useState<"sentAt" | "invoiceNumber" | "type" | "daysUntilDue">("sentAt");
  ...
  const logs = useMemo(() => {
    ...
    if (recipientFilter !== "all") filtered = filtered.filter((l) => l.recipient === recipientFilter);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      today: data.filter((l) => (l.sentAt ?? "").startsWith(new Date().toISOString().slice(0, 10))).length,
    };
  }, [q.data]);
  ...
  const SortIcon = ({ field }: { field: typeof sortField }) => {
    ...
    return <ArrowUpDown className={`h-3 w-3 ${sortDir === "asc" ? "rotate-180" : ""} text-foreground`} />;
  };

  return (
    <div>
      {/* ── Page Header ── */}
      <div className="border-b border-border bg-gradient-to-r from-background via-background to-accent/5">
        <div className="px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground/60 font-medium">
                Invoice Reminders
              </div>
              <h1 className="text-2xl font-display font-bold tracking-tight mt-0.5">Reminder History</h1>
              <p className="text-sm text-muted-foreground mt-1">
                Audit trail of every invoice reminder sent — to admins and debtors. Reminders are also sent instantly when invoices are created or updated.
              </p>
            </div>
      ...
      <div className="px-6 pt-6">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <StatCard icon={BellRing} label="Total Reminders" value={stats.total} color="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300" />
          <StatCard icon={CheckCircle} label="Delivered" value={stats.sent} sub={stats.total > 0 ? `${Math.round(stats.sent / stats.total * 100)}% success` : undefined} color="bg-emerald-100 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400" />
          <StatCard icon={XCircle} label="Failed" value={stats.failed} sub={stats.failed > 0 ? "Check SMTP config" : undefined} color="bg-red-100 text-red-600 dark:bg-red-950/30 dark:text-red-400" />
          <StatCard icon={Send} label="To Debtors" value={stats.debtors} color="bg-amber-100 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400" />
          <StatCard icon={TrendingUp} label="Overdue Alerts" value={stats.overdue} color="bg-orange-100 text-orange-600 dark:bg-orange-950/30 dark:text-orange-400" />
          <StatCard icon={Clock} label="Sent Today" value={stats.today} color="bg-blue-100 text-blue-600 dark:bg-blue-950/30 dark:text-blue-400" />
        </div>
      </div>
      ...
      <div className="px-6 pb-6 pt-3">
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          {q.isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-sm text-muted-foreground">
              ...
              <p className="text-xs mt-1">Reminders will appear here once the scheduler sends them.</p>
            </div>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    <th className="px-4 py-3 text-left">
                      <button onClick={() => toggleSort("sentAt")} className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground font-medium hover:text-foreground transition-colors">
                        <CalendarDays className="h-3 w-3" /> Date <SortIcon field="sentAt" />
                      </button>
                    </th>
                    <th className="px-4 py-3 text-left">
                      <button onClick={() => toggleSort("invoiceNumber")} className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground font-medium hover:text-foreground transition-colors">
                        <FileText className="h-3 w-3" /> Invoice <SortIcon field="invoiceNumber" />
                      </button>
                    </th>
                    <th className="px-4 py-3 text-left">
                      <button onClick={() => toggleSort("type")} className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground font-medium hover:text-foreground transition-colors">
                    ...
                    <th className="px-4 py-3 text-center">
                      <button onClick={() => toggleSort("daysUntilDue")} className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground font-medium hover:text-foreground transition-colors">
                  ...
                  {logs.map((log) => (
                    ...
                    >
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        <div className="text-[10px] text-muted-foreground mt-0.5">{daysAgo(log.sentAt)}</div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="font-mono text-sm font-medium">{log.invoiceNumber}</span>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <Badge variant={log.type}>{log.type === "sales" ? "Sales" : "Purchase"}</Badge>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          {log.isOverdue ? (
                            ...
                            <>
                              ...
                              <Badge variant="upcoming">{log.daysUntilDue === 0 ? "Due today" : `${log.daysUntilDue}d left`}</Badge>
                            </>
                          )}
                        </div>
                      </td>
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span>{logs.length} of {(q.data ?? []).length} reminders</span>
          </div>
          <div className="flex items-center gap-2">
