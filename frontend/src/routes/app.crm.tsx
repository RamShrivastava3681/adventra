...
const OPP_STAGE_PROB: Record<string, number> = { prospecting: 20, qualification: 40, proposal: 60, negotiation: 80, closed_won: 100, closed_lost: 0 };
      ...
      <PageHeader
        ...
        actions={canWrite ? (
          <div className="flex gap-2">
            <button onClick={() => setNewLead(true)} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
              <Plus className="h-4 w-4" /> Lead
            </button>
            <button onClick={() => setNewOpp(true)} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
              <Plus className="h-4 w-4" /> Opportunity
            </button>
            <button onClick={() => setNewAct(true)} className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">
...
function PipelineView() {
  ...
  const summary = useMemo(() => {
    ...
    let totalOpen = 0, weightedOpen = 0, won = 0, lost = 0;
    for (const o of opps) {
      const b = byStage.get(o.stage) ?? { count: 0, value: 0, weighted: 0 };
      b.count++; b.value += Number(o.amount); b.weighted += Number(o.amount) * (o.probability / 100);
      byStage.set(o.stage, b);
      if (o.stage === "closed_won") won += Number(o.amount);
      else if (o.stage === "closed_lost") lost += Number(o.amount);
      else { totalOpen += Number(o.amount); weightedOpen += Number(o.amount) * (o.probability / 100); }
    }
    ...
    <div className="space-y-6">
            ...
            return (
              <div key={s} className="rounded-lg border border-border bg-muted/20 p-3">
                ...
                <div className="mt-2 h-1 rounded-full bg-border">
                  <div className="h-1 rounded-full bg-primary" style={{ width: `${Math.min(100, (b.value / Math.max(1, summary.totalOpen + summary.won + summary.lost)) * 100)}%` }} />
                </div>
              </div>
            );
          })}
      ...
      <Card title="Recent opportunities">
        {oppsQ.isLoading ? (
          ...
          <div className="py-6 text-center text-sm text-muted-foreground"><Target className="mx-auto mb-2 h-8 w-8 opacity-40" />No opportunities yet.</div>
        ) : (
              ...
              <tbody>
                {(oppsQ.data ?? []).slice(0, 10).map((o: any) => (
                  <tr key={o.id} className="border-b border-border/60">
                    <td className="px-5 py-3"><div className="font-medium">{o.name}</div><div className="text-xs text-muted-foreground">{o.account_name}</div></td>
                    <td className="px-5 py-3"><StagePill stage={o.stage} /></td>
                    ...
                    <td className="px-5 py-3 text-right text-muted-foreground">{o.expected_close_date ? fmtDate(o.expected_close_date) : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function LeadsView() {
  ...
  const del = useMutation({
    mutationFn: async (id: string) => { await api.crm.leads.delete(id); },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["leads"] }); toast.success("Deleted"); },
  });

  return (
    <Card title={`Leads (${(leadsQ.data ?? []).length})`}>
      {leadsQ.isLoading ? <TableSkeleton rows={4} cols={7} />
      : (leadsQ.data ?? []).length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground"><Users className="mx-auto mb-2 h-8 w-8 opacity-40" />No leads yet.</div>
      : (
        <div className="-mx-5 overflow-x-auto">
          <table className="table-premium w-full text-sm">
            <thead className="text-xs uppercase tracking-widest text-muted-foreground">
...
function OpportunitiesView() {
  ...
  return (
    <Card title={`Opportunities (${(oppsQ.data ?? []).length})`}>
      {(oppsQ.data ?? []).length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground"><Target className="mx-auto mb-2 h-8 w-8 opacity-40" />No opportunities.</div>
      : (
        <div className="-mx-5 overflow-x-auto">
          <table className="table-premium w-full text-sm">
            <thead className="text-xs uppercase tracking-widest text-muted-foreground">
            ...
            <tbody>
              {(oppsQ.data ?? []).map((o: any) => (
                <tr key={o.id} className="border-b border-border/60 hover:bg-muted/30">
                  ...
                  <td className="px-5 py-3 text-right num">{fmtMoney(o.amount)}</td>
                  <td className="px-5 py-3 text-right num text-muted-foreground">{o.probability}%</td>
                  <td className="px-5 py-3 text-right num text-primary">{fmtMoney(Number(o.amount) * (o.probability / 100))}</td>
                  <td className="px-5 py-3 text-right text-muted-foreground">{o.expected_close_date ? fmtDate(o.expected_close_date) : "—"}</td>
                </tr>
              ))}
            </tbody>
...
function ActivitiesView() {
  ...
  const toggle = useMutation({
    mutationFn: async (a: any) => {
      await api.crm.activities.update(a.id, { completed: !a.completed, completed_at: !a.completed ? new Date().toISOString() : null });
    },
  ...
  return (
    <Card title={`Activities (${(actsQ.data ?? []).length})`}>
      {(actsQ.data ?? []).length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground"><Calendar className="mx-auto mb-2 h-8 w-8 opacity-40" />No activities.</div>
      : (
...
function LeadModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  const qc = useQueryClient();
  const [f, setF] = useState({ name: "", company: "", email: "", phone: "", source: "other", status: "new", estimated_value: "", notes: "" });
    ...
    <ModalShell title="New lead" onClose={onClose}>
      <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="space-y-4 p-5">
        <div className="grid grid-cols-2 gap-3">
          <L label="Name *"><input required className="inp" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /></L>
          <L label="Company"><input className="inp" value={f.company} onChange={(e) => setF({ ...f, company: e.target.value })} /></L>
          <L label="Email"><input type="email" className="inp" value={f.email} onChange={(e) => setF({ ...f, email: e.target.value })} /></L>
          <L label="Phone"><input className="inp" value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} /></L>
          <L label="Source"><select className="inp" value={f.source} onChange={(e) => setF({ ...f, source: e.target.value })}>{LEAD_SOURCES.map((s) => <option key={s} value={s}>{s}</option>)}</select></L>
          <L label="Estimated value"><input type="number" step="0.01" className="inp" value={f.estimated_value} onChange={(e) => setF({ ...f, estimated_value: e.target.value })} /></L>
        </div>
        <L label="Notes"><textarea rows={3} className="inp" value={f.notes} onChange={(e) => setF({ ...f, notes: e.target.value })} /></L>
...
function OppModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  const qc = useQueryClient();
  const leadsQ = useQuery({ queryKey: ["leads-mini"], queryFn: async () => {
    const data = await api.crm.leads.list();
    return data.map((l: any) => ({ id: l.id, name: l.name, company: l.company }));
  } });
  const [f, setF] = useState({ name: "", account_name: "", lead_id: "", stage: "prospecting", amount: "", expected_close_date: "", notes: "" });
  const create = useMutation({
    mutationFn: async () => {
      if (!f.name.trim()) throw new Error("Name required");
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["opportunities"] }); toast.success("Opportunity added"); onClose(); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  return (
...
function ActivityModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  const qc = useQueryClient();
  const leadsQ = useQuery({ queryKey: ["leads-mini2"], queryFn: async () => {
    const data = await api.crm.leads.list();
    return data.map((l: any) => ({ id: l.id, name: l.name }));
  } });
  const oppsQ = useQuery({ queryKey: ["opps-mini"], queryFn: async () => {
    const data = await api.crm.opportunities.list();
    return data.map((o: any) => ({ id: o.id, name: o.name }));
  } });
  const [f, setF] = useState({ activity_type: "call", subject: "", description: "", due_date: "", lead_id: "", opportunity_id: "" });
  const create = useMutation({
    mutationFn: async () => {
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["crm_activities"] }); toast.success("Activity added"); onClose(); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  return (
...
function ModalShell({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-border bg-card" onClick={(e) => e.stopPropagation()}>
...
function StatTile({ label, value, tone }: { label: string; value: string | number; tone?: "success" | "warning" | "destructive" | "primary" }) {
  const t = tone === "success" ? "text-success" : tone === "warning" ? "text-warning" : tone === "destructive" ? "text-destructive" : tone === "primary" ? "text-primary" : "text-foreground";
  return (
...
function StagePill({ stage }: { stage: string }) {
  const tone = stage === "closed_won" ? "success" : stage === "closed_lost" ? "destructive" : stage === "negotiation" || stage === "proposal" ? "primary" : "warning";
  ...
  return <span className={`inline-block rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-widest ${s}`}>{stage.replace("_", " ")}</span>;
}
