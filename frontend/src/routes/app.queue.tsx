...
function QueuePage() {
  ...
  // ── Payment / receipt history (closed invoices) ──
  const historyQ = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const i of sales) {
        const s = i as any;
        if (s.status === "paid" || s.status === "partially_paid") {
          rows.push({
            kind: "sale",
            invoice_number: s.invoice_number,
            party: partyMap[s.debtor_id] ?? "—",
            amount: Number(s.amount ?? s.grand_total ?? 0),
            amount_paid: Number(s.amountReceived) || 0,
      ...
      for (const p of purchases) {
        const pi = p as any;
        if (pi.status === "paid" || pi.status === "partially_paid") {
          rows.push({
            kind: "purchase",
            invoice_number: pi.invoice_number,
            party: pi.supplier_name ?? "—",
            amount: Number(pi.amount ?? pi.grand_total ?? 0),
      ...
      rows.sort((a, b) => {
        const da = a.paid_date ? new Date(a.paid_date).getTime() : 0;
        const db = b.paid_date ? new Date(b.paid_date).getTime() : 0;
        return db - da;
      });
      return rows;
    },
                ...
                <tbody>
                  {(historyQ.data ?? []).map((h: any, idx: number) => (
                    <tr key={idx} className="border-b border-border/60 hover:bg-muted/30">
                      ...
                      <td className="px-5 py-3 font-mono text-xs">{h.invoice_number}</td>
                      <td className="px-5 py-3 text-muted-foreground">{h.party}</td>
                      <td className="px-5 py-3 text-right num">{fmtMoney(h.amount)}</td>
                      <td className="px-5 py-3 text-right num">{fmtMoney(h.amount_paid)}</td>
                      <td className="px-5 py-3">
