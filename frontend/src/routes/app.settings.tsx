...
function SettingsPage() {
  ...
  useEffect(() => {
    ...
    api.auth.me().then((data: any) => {
      if (data) setProfile({ company_name: data.companyName ?? data.company_name ?? "", contact_name: data.contactName ?? data.contact_name ?? "" });
    }).catch(() => {});
  }, [user]);
  ...
  const save = async () => {
    setLoading(true);
    try {
      await api.auth.updateProfile({ company_name: profile.company_name, contact_name: profile.contact_name });
              ...
              {!isAdmin && (
                <div className="mt-4 rounded-md border border-warning/40 bg-warning/10 p-3 text-xs">
                  ...
                  <p className="mt-1 text-muted-foreground">
                    For demonstration purposes, you can grant yourself factor admin to explore the operations console.
                    In production, this would be granted out-of-band.
                  </p>
                  <button onClick={becomeAdmin} disabled={loading} className="mt-3 rounded-md border border-warning/40 px-3 py-1.5 text-xs text-warning hover:bg-warning/10">
