...
function computeBS(asOf: string, data: Awaited<ReturnType<typeof fetchBalanceSheetData>>) {
  ...
  for (const j of journals as any[]) {
    for (const l of j.lines ?? []) {
      ...
      arr.push({ date: j.journal_date, ref: j.reference || "JRN", description: l.description || j.description || "Journal entry", amount: amt });
  ...
  const manualLines = (s: Section): LineItem[] =>
    manual.filter((m) => m.section === s).map((m) => ({
      key: `manual-${m.id}`,
      label: m.name,
      amount: Number(m.amount || 0),
      rows: [{ date: m.entry_date, ref: m.name, description: m.description || "Manual entry", amount: Number(m.amount || 0) }],
    }));

  // ---------- Fixed Assets (Tangible) ----------
  const tangibleAccts: LineItem[] = coa
    .filter((a) => a.type === "asset" && a.subtype === "fixed")
    .map((a) => ({ key: a.id, label: a.name, amount: acctBalance(a.id, true), rows: acctDrill(a.id, true) }));
    ...
    .map((a) => {
      ...
      if (a.system_key === "bank") {
        ...
        for (const adv of advances) {
          ...
          flow += v;
          rows.push({ date: adv.advance_date, ref: adv.payment_ref, description: adv.reference || (adv.side === "sales" ? "Customer receipt" : "Supplier payment"), amount: v });
        }
        amt += flow;
      }
  ...
  for (const a of advances) if (a.side === "sales" && a.invoice_id) paidByInv.set(a.invoice_id, (paidByInv.get(a.invoice_id) ?? 0) + Number(a.amount || 0));
  ...
  for (const i of invoices as any[]) {
    ...
    arRows.push({ date: i.issue_date, ref: i.invoice_number, description: `Invoice ${i.invoice_number}`, amount: outstanding });
    arTotal += outstanding;
  }
  ...
  const invByKey = new Map<string, { item: string; qty: number; totalCost: number; unitAvg: number }>();
  const sorted = [...stock].sort((x: any, y: any) => (x.created_at || "").localeCompare(y.created_at || ""));
  ...
  for (const v of invByKey.values()) {
    ...
    invRows.push({ date: null, ref: v.item, description: `${v.qty.toFixed(2)} × ${v.unitAvg.toFixed(2)}`, amount: val });
    invTotal += val;
  }

  // Exclude fixed / cash / AR / inventory accounts — the rest are "other current assets"
  const otherAssetAccts = coa.filter((a) =>
    a.type === "asset" &&
  ...
  const otherCA: LineItem[] = [
    ...
    ...(invTotal > 0.005 ? [{ key: "inventory", label: "Inventory", amount: invTotal, rows: invRows }] : []),
    ...otherAssetAccts.map((a) => ({ key: a.id, label: a.name, amount: acctBalance(a.id, true), rows: acctDrill(a.id, true) })),
  ...
  for (const a of advances) if (a.side !== "sales" && a.purchase_invoice_id) paidByBill.set(a.purchase_invoice_id, (paidByBill.get(a.purchase_invoice_id) ?? 0) + Number(a.amount || 0));
  const apRows: DrillRow[] = [];
  let apTotal = 0;
  for (const b of bills as any[]) {
    const outstanding = Number(b.amount || 0) - (paidByBill.get(b.id) ?? 0);
    if (outstanding <= 0.005) continue;
    apRows.push({ date: b.issue_date, ref: b.invoice_number, description: `Bill ${b.invoice_number}`, amount: outstanding });
    apTotal += outstanding;
  }
  ...
  for (const a of advances) {
    if (a.side === "sales" && !a.invoice_id) {
      custAdvRows.push({ date: a.advance_date, ref: a.payment_ref, description: a.reference || "Customer advance", amount: Number(a.amount || 0) });
  ...
  const corpTaxAcct = coa.find((a) => a.type === "liability" && (a.system_key === "corp_tax" || /corporation\s*tax/i.test(a.name)));
  ...
  const roundingAcct = coa.find((a) => a.type === "liability" && (a.system_key === "rounding" || /rounding/i.test(a.name)));
  ...
  const excludedLiabIds = new Set(
    [
      ...coa.filter((a) => a.type === "liability" && a.subtype === "accounts_payable").map((a) => a.id),
      corpTaxAcct?.id,
      roundingAcct?.id,
  ...
  const otherCL: LineItem[] = [
    ...otherLiabAccts.map((a) => ({ key: a.id, label: a.name, amount: acctBalance(a.id, false), rows: acctDrill(a.id, false) })),
  ...
  let revenue = 0, cogs = 0, expenseSum = 0;
  for (const i of invoices as any[]) {
    if (!APPROVED.has(String(i.status))) continue;
    if (i.issue_date >= fyStart) {
      revenue += Number(i.amount || 0);
      cyeRows.push({ date: i.issue_date, ref: i.invoice_number, description: `Sales — ${i.invoice_number}`, amount: Number(i.amount || 0) });
    }
  }
  for (const b of bills as any[]) {
    if (b.issue_date >= fyStart) {
      cogs += Number(b.amount || 0);
      cyeRows.push({ date: b.issue_date, ref: b.invoice_number, description: `Purchase — ${b.invoice_number}`, amount: -Number(b.amount || 0) });
    }
  }
  for (const e of expenses as any[]) {
    if (e.expense_date >= fyStart) {
      expenseSum += Number(e.amount || 0);
      cyeRows.push({ date: e.expense_date, ref: e.expense_ref, description: e.description || e.category || "Expense", amount: -Number(e.amount || 0) });
    }
  }
  ...
  const shareAccts = coa.filter((a) => a.type === "equity" && (a.subtype === "capital" || a.subtype === "share_capital"));
  const share: LineItem[] = [
    ...shareAccts.map((a) => ({ key: a.id, label: a.name, amount: acctBalance(a.id, false), rows: acctDrill(a.id, false) })),
  ...
  const otherEquityAccts = coa.filter((a) => a.type === "equity" && a.system_key !== "retained" && a.subtype !== "capital" && a.subtype !== "share_capital");
  const otherEquity: LineItem[] = [
    ...otherEquityAccts.map((a) => ({ key: a.id, label: a.name, amount: acctBalance(a.id, false), rows: acctDrill(a.id, false) })),
...
function BalanceSheetPage() {
  ...
  const compareAsOf = useMemo(() => {
    const d = new Date(asOf); d.setFullYear(d.getFullYear() - 1);
  ...
  const compare = useQuery({ queryKey: ["bs", compareAsOf], queryFn: () => fetchBalanceSheetData(compareAsOf), enabled: compareOn });

  useEffect(() => {
    const tables = ["invoices", "purchase_invoices", "advances", "stock_movements", "chart_of_accounts", "journals", "journal_lines", "manual_balance_entries", "expenses"];
  ...
  const bs = useMemo(() => (primary.data ? computeBS(asOf, primary.data) : null), [primary.data, asOf]);
  const bsCmp = useMemo(() => (compare.data ? computeBS(compareAsOf, compare.data) : null), [compare.data, compareAsOf]);
  ...
  const doExport = (kind: "csv" | "xls" | "pdf") => {
    ...
    if (!isBalanced && kind !== "pdf") {
      if (!confirm(`Balance Sheet is out of balance by ${fmtAccounting(diff)}. Export anyway?`)) return;
    }
    ...
    <div>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2 print:hidden">
            <button onClick={() => doExport("csv")} className="inline-flex items-center gap-2 rounded-md border border-border bg-background px-3 py-1.5 text-sm hover:bg-muted"><Download className="h-4 w-4" /> CSV</button>
            <button onClick={() => doExport("xls")} className="inline-flex items-center gap-2 rounded-md border border-border bg-background px-3 py-1.5 text-sm hover:bg-muted"><FileSpreadsheet className="h-4 w-4" /> Excel</button>
            <button onClick={() => doExport("pdf")} className="inline-flex items-center gap-2 rounded-md border border-border bg-background px-3 py-1.5 text-sm hover:bg-muted"><Printer className="h-4 w-4" /> PDF</button>
          </div>
        }
      />
      ...
      <div className="px-6 py-4">
        <div className="mb-4 flex flex-wrap items-end gap-4 print:hidden">
          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground">As of</label>
            <input type="date" value={asOf} onChange={(e) => setAsOf(e.target.value)} className="block rounded-md border border-border bg-background px-3 py-1.5 text-sm" />
          </div>
        ...
        {!isBalanced && bs && (
          <div className="mb-4 flex items-start gap-3 rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">
            ...
            <div>
              ...
              <div>Total Assets − (Total Liabilities + Total Capital and Reserves) = <span className="font-mono">{fmtAccounting(diff)}</span>.</div>
            </div>
          </div>
        )}

        {primary.isLoading || !bs ? (
          <div className="rounded-md border border-border p-8 text-center text-sm text-muted-foreground">Loading…</div>
        ) : (
          <div id="bs-print" className="max-w-4xl">
            <Card>
              ...
              {bs.tangible.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("tangible", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}
              <Total label="Total Tangible Assets" value={bs.totalTangible} compare={bsCmp?.totalTangible} indent={1} />
              ...
              {bs.cash.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("cash", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}
              <Total label="Total Cash at bank and in hand" value={bs.totalCash} compare={bsCmp?.totalCash} indent={1} />

              <SubGroup title="Accounts Receivable" />
              <Line label="Accounts Receivable" value={bs.ar.total} compare={bsCmp?.ar.total} onClick={() => openDrill("Accounts Receivable", bs.ar.rows)} indent={2} />

              <SubGroup title="Other Current Assets">
                <button onClick={() => { setEditEntry(null); setManualOpen("other_current_asset"); }} title="Add manual current asset" className="print:hidden inline-flex items-center rounded border border-border bg-background p-0.5 text-muted-foreground hover:text-foreground">
                  <Plus className="h-3 w-3" />
                </button>
              </SubGroup>
              {bs.otherCA.length === 0 && <EmptyLine />}
              {bs.otherCA.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("otherCA", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}
              <Total label="Total Current Assets" value={bs.totalCurrentAssets} compare={bsCmp?.totalCurrentAssets} />

              {/* ===== Creditors ===== */}
              <Group title="Creditors: amounts falling due within one year" />
              <Line label="Accounts Payable" value={bs.ap.total} compare={bsCmp?.ap.total} onClick={() => openDrill("Accounts Payable", bs.ap.rows)} indent={1} />
              <Line label="Advance received from Customers" value={bs.custAdv.total} compare={bsCmp?.custAdv.total} onClick={() => openDrill("Advance from Customers", bs.custAdv.rows)} indent={1} />
              <Line label={bs.corpTax.name} value={bs.corpTax.total} compare={bsCmp?.corpTax.total} onClick={() => openDrill(bs.corpTax.name, bs.corpTax.rows)} indent={1} />
              <Line label={bs.rounding.name} value={bs.rounding.total} compare={bsCmp?.rounding.total} onClick={() => openDrill(bs.rounding.name, bs.rounding.rows)} indent={1} />

              <SubGroup title="Other Current Liabilities">
                <button onClick={() => { setEditEntry(null); setManualOpen("other_current_liability"); }} title="Add manual current liability" className="print:hidden inline-flex items-center rounded border border-border bg-background p-0.5 text-muted-foreground hover:text-foreground">
                  <Plus className="h-3 w-3" />
                </button>
              </SubGroup>
              {bs.otherCL.length === 0 && <EmptyLine />}
              {bs.otherCL.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("otherCL", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}
              <Total label="Total Creditors: amounts falling due within one year" value={bs.totalCreditors} compare={bsCmp?.totalCreditors} />

              {/* ===== Net Current Assets ===== */}
              <Total label="Net Current Assets (Liabilities)" value={bs.netCurrentAssets} compare={bsCmp?.netCurrentAssets} spaced />

              {/* ===== Total Assets Less Current Liabilities ===== */}
              <Total label="Total Assets less Current Liabilities" value={bs.totalAssetsLessCL} compare={bsCmp?.totalAssetsLessCL} spaced />

              {/* ===== Net Assets ===== */}
              <Total label="Net Assets" value={bs.netAssets} compare={bsCmp?.netAssets} strong spaced />

              {/* ===== Capital and Reserves ===== */}
              <Group title="Capital and Reserves" />
              <Line label="Current Year Earnings" value={bs.cye.total} compare={bsCmp?.cye.total} onClick={() => openDrill("Current Year Earnings", bs.cye.rows)} indent={1} />
              <Line label="Retained Earnings" value={bs.retained.total} compare={bsCmp?.retained.total} onClick={() => openDrill("Retained Earnings", bs.retained.rows)} indent={1} />

              <SubGroup title="Share Capital" />
              {bs.share.length === 0 && <EmptyLine />}
              {bs.share.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("share", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}

              <SubGroup title="Other Equity">
                <button onClick={() => { setEditEntry(null); setManualOpen("other_equity"); }} title="Add manual equity" className="print:hidden inline-flex items-center rounded border border-border bg-background p-0.5 text-muted-foreground hover:text-foreground">
                  <Plus className="h-3 w-3" />
                </button>
              </SubGroup>
              {bs.otherEquity.length === 0 && <EmptyLine />}
              {bs.otherEquity.map((li) => (
                <Line key={li.key} label={li.label} value={li.amount} compare={cmpAmount("otherEquity", li.key)} onClick={() => openDrill(li.label, li.rows)} indent={2} />
              ))}

              <Total label="Total Capital and Reserves" value={bs.totalCapitalReserves} compare={bsCmp?.totalCapitalReserves} strong spaced />
            </Card>
          </div>
        )}
      </div>
      ...
      {manualOpen && user && (
        <ManualEntryModal
          ...
          entry={editEntry}
          clientId={user.id}
          onClose={() => { setManualOpen(null); setEditEntry(null); }}
...
function Group({ title }: { title: string }) {
  return <div className="mt-5 mb-1 border-b border-border pb-1 text-sm font-semibold uppercase tracking-wider">{title}</div>;
}
...
function Line({ label, value, compare, onClick, indent = 0 }: { label: string; value: number; compare?: number; onClick?: () => void; indent?: number }) {
  const pad = { 0: "", 1: "pl-4", 2: "pl-8", 3: "pl-12" }[indent] ?? "";
  return (
    <div className={`flex items-center justify-between border-b border-border/30 py-1 text-sm ${pad}`}>
      <button onClick={onClick} className="text-left hover:text-primary underline-offset-2 hover:underline">{label}</button>
      <div className="flex items-center gap-8 font-mono">
        {compare !== undefined && <span className="w-28 text-right text-muted-foreground">{fmtAccounting(compare)}</span>}
        <span className="w-32 text-right">{fmtAccounting(value)}</span>
      </div>
    </div>
  );
}
function Total({ label, value, compare, strong, spaced, indent = 0 }: { label: string; value: number; compare?: number; strong?: boolean; spaced?: boolean; indent?: number }) {
  const pad = { 0: "", 1: "pl-4", 2: "pl-8" }[indent] ?? "";
  return (
    <div className={`flex items-center justify-between border-t ${strong ? "border-t-2 border-foreground" : "border-border"} ${spaced ? "mt-3" : "mt-1"} pt-1.5 text-sm font-semibold ${pad}`}>
      <span>{label}</span>
      <div className="flex items-center gap-8 font-mono">
        {compare !== undefined && <span className="w-28 text-right text-muted-foreground">{fmtAccounting(compare)}</span>}
        <span className={`w-32 text-right ${strong ? "text-base" : ""}`}>{fmtAccounting(value)}</span>
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 print:hidden">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-xl overflow-auto border-l border-border bg-background p-5 shadow-xl">
        ...
        {drill.rows.length ? (
          ...
          <div className="rounded-md border border-border p-6 text-center text-sm text-muted-foreground">No transactions.</div>
        )}
      </div>
    </div>
  );
...
function ManualEntryModal({ section, entry, clientId, onClose, onEdit }: { section: Section; entry: ManualEntry | null; clientId: string; onClose: () => void; onEdit: (e: ManualEntry | null) => void }) {
  const qc = useQueryClient();
  const list = useQuery({
  ...
  const save = useMutation({
    mutationFn: async () => {
      const payload = {
        client_id: clientId, section, name,
    ...
    onSuccess: () => {
      ...
      onEdit(null);
      setName(""); setDescription(""); setAmount(""); setAccountId(""); setNotes("");
    },
    ...
    <div className="fixed inset-0 z-50 grid place-items-center print:hidden">
      ...
      <div className="relative m-4 w-full max-w-3xl rounded-xl border border-border bg-background p-6 shadow-2xl">
        ...
        <div className="mb-6 rounded-lg border border-border p-4">
          <div className="mb-3 text-xs uppercase tracking-widest text-muted-foreground">{entry ? "Edit entry" : "New entry"}</div>
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="Name *"><input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm" /></Field>
          ...
          <div className="mt-3 flex justify-end gap-2">
            {entry && <button onClick={() => onEdit(null)} className="rounded-md border border-border bg-background px-3 py-1.5 text-sm">Cancel edit</button>}
            <button
        ...
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-muted-foreground">Existing entries</div>
          <table className="table-premium w-full text-sm">
            <thead className="text-left text-xs uppercase text-muted-foreground border-b border-border">
              <tr>
                <th className="py-2">Date</th>
              ...
              {(list.data ?? []).map((m) => (
                <tr key={m.id} className="border-b border-border/30">
                  ...
                  <td className="py-1.5 text-right">
                    <button onClick={() => onEdit(m)} className="mr-2 text-xs text-primary hover:underline">Edit</button>
                    <button onClick={() => del.mutate(m.id)} className="text-xs text-destructive hover:underline"><Trash2 className="inline h-3 w-3" /></button>
                  </td>
                </tr>
              ))}
              {!list.data?.length && (
                <tr><td colSpan={5} className="py-6 text-center text-sm text-muted-foreground">No entries yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
/* ========== Export helpers ========== */
type ExportRow = { label: string; value: number; compare?: number; kind?: "header" | "sub" | "total" | "grand" };
function flattenForExport(bs: BS, asOf: string, cmp: BS | null | undefined, cmpDate: string): ExportRow[] {
  const rows: ExportRow[] = [];
  const at = (arr: LineItem[] | undefined, key: string) => arr?.find((r) => r.key === key)?.amount;
  rows.push({ label: `Balance Sheet as of ${asOf}${cmp ? ` (vs ${cmpDate})` : ""}`, value: 0, kind: "header" });

  rows.push({ label: "Fixed Assets", value: 0, kind: "header" });
  rows.push({ label: "  Tangible Assets", value: 0, kind: "sub" });
  bs.tangible.forEach((li) => rows.push({ label: `    ${li.label}`, value: li.amount, compare: at(cmp?.tangible, li.key) }));
  rows.push({ label: "  Total Tangible Assets", value: bs.totalTangible, compare: cmp?.totalTangible, kind: "total" });
  rows.push({ label: "Total Fixed Assets", value: bs.totalFixed, compare: cmp?.totalFixed, kind: "total" });

  rows.push({ label: "Current Assets", value: 0, kind: "header" });
  rows.push({ label: "  Cash at bank and in hand", value: 0, kind: "sub" });
  bs.cash.forEach((li) => rows.push({ label: `    ${li.label}`, value: li.amount, compare: at(cmp?.cash, li.key) }));
  rows.push({ label: "  Total Cash at bank and in hand", value: bs.totalCash, compare: cmp?.totalCash, kind: "total" });
  ...
  bs.otherCA.forEach((li) => rows.push({ label: `    ${li.label}`, value: li.amount, compare: at(cmp?.otherCA, li.key) }));
  rows.push({ label: "Total Current Assets", value: bs.totalCurrentAssets, compare: cmp?.totalCurrentAssets, kind: "total" });
  ...
  rows.push({ label: "  Advance received from Customers", value: bs.custAdv.total, compare: cmp?.custAdv.total });
  rows.push({ label: `  ${bs.corpTax.name}`, value: bs.corpTax.total, compare: cmp?.corpTax.total });
  rows.push({ label: `  ${bs.rounding.name}`, value: bs.rounding.total, compare: cmp?.rounding.total });
  bs.otherCL.forEach((li) => rows.push({ label: `  ${li.label}`, value: li.amount, compare: at(cmp?.otherCL, li.key) }));
  rows.push({ label: "Total Creditors: amounts falling due within one year", value: bs.totalCreditors, compare: cmp?.totalCreditors, kind: "total" });

  rows.push({ label: "Net Current Assets (Liabilities)", value: bs.netCurrentAssets, compare: cmp?.netCurrentAssets, kind: "total" });
  rows.push({ label: "Total Assets less Current Liabilities", value: bs.totalAssetsLessCL, compare: cmp?.totalAssetsLessCL, kind: "total" });
  ...
  rows.push({ label: "  Retained Earnings", value: bs.retained.total, compare: cmp?.retained.total });
  rows.push({ label: "  Share Capital", value: 0, kind: "sub" });
  bs.share.forEach((li) => rows.push({ label: `    ${li.label}`, value: li.amount, compare: at(cmp?.share, li.key) }));
  rows.push({ label: "  Other Equity", value: 0, kind: "sub" });
  bs.otherEquity.forEach((li) => rows.push({ label: `    ${li.label}`, value: li.amount, compare: at(cmp?.otherEquity, li.key) }));
  rows.push({ label: "Total Capital and Reserves", value: bs.totalCapitalReserves, compare: cmp?.totalCapitalReserves, kind: "grand" });
  return rows;
}
...
function toCSV(rows: ExportRow[]) {
  ...
  const body = rows.map((r) => r.kind === "header" || r.kind === "sub"
    ? esc(r.label)
    : hasCompare
...
function toXLS(rows: ExportRow[]) {
  ...
  const cells = rows.map((r) => {
    if (r.kind === "header" || r.kind === "sub") return `<tr><td colspan="${hasCompare ? 3 : 2}" style="font-weight:bold;background:#eef">${escapeHtml(r.label)}</td></tr>`;
    const weight = r.kind === "grand" ? "font-weight:bold;border-top:2px solid #000" : r.kind === "total" ? "font-weight:bold;border-top:1px solid #999" : "";
    const compare = hasCompare ? `<td style="text-align:right;${weight}">${r.compare != null ? r.compare.toFixed(2) : ""}</td>` : "";
